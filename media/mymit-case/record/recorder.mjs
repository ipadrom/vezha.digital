// Screen recorder for MyMIT case videos: CDP screencast frames -> ffmpeg mp4.
...
export const OUT = process.env.OUT_DIR || fileURLToPath(new URL('../../../frontend/public/cases/mymit/2026-09/', import.meta.url));
...
export async function start({ width = 560, height = 1000, dpr = 2, headless = true } = {}) {
  ...
  const context = await browser.newContext({ viewport: { width, height }, deviceScaleFactor: dpr, locale: 'ru-RU', timezoneId: 'Europe/Moscow', hasTouch: false });
  ...
  async function record(name) {
    ...
    rec = { name, frames: [], running: true, vp };
    const cur = rec;
    ...
    cur.loop = (async () => {
      while (cur.running) {
        ...
        try {
          ...
          const { data } = await cdp.send('Page.captureScreenshot', { format: 'jpeg', quality: 92, clip: { x: sx, y: sy, width: vp.width, height: vp.height, scale: dpr } });
  ...
  return { browser, context, page, cdp, sleep, moveTo, tap, type, hideCursor, record, stop, region, close, width, height };
}
