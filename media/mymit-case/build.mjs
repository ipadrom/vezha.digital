...
const chapters = [
  ...
  ['Уведомления', 'Notifications',
   ...
   'Developed browser notifications and spoken announcements in booths.', [
    ...
    ['Голосовые объявления по событиям брони', 'Booking-triggered announcements',
     'Добавили административную настройку голосовых событий и связь с Яндекс-колонками. Объявления привязаны к жизненному циклу брони: началу, приближению окончания и завершению. Для конкретной колонки используется проверенный сценарий; администратор задаёт событие и момент его срабатывания.',
     'Added admin configuration for voice events and integration with Yandex speakers. Announcements follow the booking lifecycle, including start, upcoming end and completion. Each speaker uses a verified scenario, with events and timing configured by an administrator.', 'speaker-events.png'],
  ]],
  ['Кабинка и приложение', 'Booth and application',
   ...
   'Improved backend equipment integration and device-state handling.', [
    ['Подключение и доступ устройств', 'Device connection and access',
     'Добавили выбор MQTT-провайдера для каждого устройства и параллельную поддержку собственного брокера и прежней интеграции. Расширили администрирование выдачей, отключением и восстановлением доступа. Настройки разделены по устройствам, поэтому новое подключение не требует одновременно переводить весь парк.',
     'Added per-device MQTT provider selection and support for the custom broker alongside the legacy integration. Extended admin controls for issuing, disabling and restoring access. Per-device settings avoid requiring a simultaneous fleet-wide change.', 'device-provider.png'],
