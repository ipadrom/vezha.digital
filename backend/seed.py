...
async def seed_services(session):
    services = [
        Service(
            ...
            description_ru="Разрабатываем мини-приложения внутри Telegram: от простого каталога до полноценного сервиса с оплатой и личным кабинетом.",
            description_en="We build mini-apps inside Telegram: from a simple catalog to a full-featured service with payments and user accounts.",
            examples_ru="Магазин,Каталог товаров,Сервис бронирования,Система лояльности,Доска объявлений,Игровое приложение,Сервис доставки,Приложение знакомств",
            examples_en="Store,Product Catalog,Booking Service,Loyalty System,Classifieds,Game App,Delivery Service,Dating App",
        ...
        Service(
            ...
            description_ru="Автоматизируем рутину через Telegram: приём заявок, запись клиентов, рассылки, AI-ассистенты и любая логика под вашу задачу.",
            description_en="We automate routine via Telegram: order intake, client booking, newsletters, AI assistants and any logic for your task.",
            examples_ru="Приём заказов,Запись на услуги,AI-ассистент,Рассылки,Бот с оплатой,Управление группой,Опросы и викторины",
            examples_en="Order Intake,Service Booking,AI Assistant,Newsletters,Payment Bot,Group Management,Polls & Quizzes",
        ...
        Service(
            ...
            name_ru="Веб-сайты",
            name_en="Websites",
            description_ru="Делаем сайты, которые работают на всех устройствах: лендинги, корпоративные сайты, каталоги и промо-страницы.",
            description_en="We build sites that work on all devices: landing pages, corporate sites, catalogs and promo pages.",
            examples_ru="Лендинг,Сайт-визитка,Промо-сайт,Портфолио,Корпоративный сайт,Каталог,Блог",
            examples_en="Landing Page,Business Card Site,Promo Site,Portfolio,Corporate Site,Catalog,Blog",
        ...
        Service(
            ...
            name_en="E-commerce",
            description_ru="Магазин, который удобно использовать покупателю и легко администрировать вам.",
            description_en="A store that's convenient for buyers and easy for you to manage.",
            examples_ru="До 100 товаров,До 500 товаров,Крупный магазин с аналитикой",
            examples_en="Up to 100 products,Up to 500 products,Large store with analytics",
        ...
        Service(
            ...
            description_ru="Внедряем AI туда, где это реально экономит время: чат-боты, обработка данных, автоматизация рутинных процессов.",
            description_en="We integrate AI where it really saves time: chatbots, data processing, routine process automation.",
            examples_ru="AI чат-бот,Помощник для сайта,Анализ текста,Система рекомендаций,Обработка изображений,Автоматизация процессов",
            examples_en="AI Chatbot,Site Assistant,Text Analysis,Recommendation System,Image Processing,Process Automation",
        ...
        Service(
            ...
            description_ru="Инструмент, который работает именно под ваши процессы, а не требует подстраиваться под готовое решение.",
            description_en="A tool built around your processes, not one you have to adapt to.",
            examples_ru="Учёт рабочего времени,Внутренний портал,Складская система,Аналитика,Управление проектами,HR-система",
            examples_en="Time Tracking,Internal Portal,Warehouse System,Analytics,Project Management,HR System",
        ...
        Service(
            ...
            description_ru="PWA-приложение, которое работает на iOS и Android без публикации в магазинах и долгой модерации.",
            description_en="A PWA that works on iOS and Android without app store submission or lengthy review.",
            examples_ru="PWA-приложение,Установка на экран,Офлайн-режим,Push-уведомления",
            examples_en="PWA App,Home Screen Install,Offline Mode,Push Notifications",
...
async def seed_service_features(session, services):
    ...
    features = [
        # Telegram Mini Apps
        ServiceFeature(service_id=services[0].id, text_ru="Работает прямо в Telegram без установки", text_en="Works directly in Telegram without installation", sort_order=0),
        ServiceFeature(service_id=services[0].id, text_ru="Интеграция платёжных систем", text_en="Payment system integration", sort_order=1),
        ServiceFeature(service_id=services[0].id, text_ru="Авторизация через Telegram", text_en="Telegram authorization", sort_order=2),
        ServiceFeature(service_id=services[0].id, text_ru="Административная панель", text_en="Admin panel", sort_order=3),
        # Telegram боты
        ServiceFeature(service_id=services[1].id, text_ru="Интеграция с внешними API", text_en="External API integration", sort_order=0),
        ServiceFeature(service_id=services[1].id, text_ru="Работа с базой данных", text_en="Database integration", sort_order=1),
        ServiceFeature(service_id=services[1].id, text_ru="Административная панель", text_en="Admin panel", sort_order=2),
        ServiceFeature(service_id=services[1].id, text_ru="Мультиязычность", text_en="Multilingual support", sort_order=3),
        # Веб-сайты
        ServiceFeature(service_id=services[2].id, text_ru="Адаптивный дизайн для всех устройств", text_en="Responsive design for all devices", sort_order=0),
        ServiceFeature(service_id=services[2].id, text_ru="SEO оптимизация из коробки", text_en="SEO optimization out of the box", sort_order=1),
        ServiceFeature(service_id=services[2].id, text_ru="Быстрая загрузка страниц", text_en="Fast page loading", sort_order=2),
        ServiceFeature(service_id=services[2].id, text_ru="Интеграция с формами обратной связи", text_en="Contact form integration", sort_order=3),
        # Интернет-магазины
        ServiceFeature(service_id=services[3].id, text_ru="Корзина и оформление заказа", text_en="Cart and checkout", sort_order=0),
        ServiceFeature(service_id=services[3].id, text_ru="Интеграция платёжных систем", text_en="Payment system integration", sort_order=1),
        ServiceFeature(service_id=services[3].id, text_ru="Личный кабинет покупателя", text_en="Customer account", sort_order=2),
        ServiceFeature(service_id=services[3].id, text_ru="Административная панель", text_en="Admin panel", sort_order=3),
        # AI и автоматизация
        ServiceFeature(service_id=services[4].id, text_ru="Обучение на ваших данных", text_en="Training on your data", sort_order=0),
        ServiceFeature(service_id=services[4].id, text_ru="Интеграция в существующие системы", text_en="Integration into existing systems", sort_order=1),
        ServiceFeature(service_id=services[4].id, text_ru="Создание кастомных AI решений", text_en="Custom AI solutions", sort_order=2),
        ServiceFeature(service_id=services[4].id, text_ru="YandexGPT, GigaChat и другие модели", text_en="YandexGPT, GigaChat and other models", sort_order=3),
        # Корпоративные системы
        ServiceFeature(service_id=services[5].id, text_ru="Интеграция с 1С и Битрикс24", text_en="Integration with 1C and Bitrix24", sort_order=0),
        ServiceFeature(service_id=services[5].id, text_ru="OAuth авторизация", text_en="OAuth authorization", sort_order=1),
        ServiceFeature(service_id=services[5].id, text_ru="REST API для сторонних систем", text_en="REST API for third-party systems", sort_order=2),
        ServiceFeature(service_id=services[5].id, text_ru="Административная панель", text_en="Admin panel", sort_order=3),
        # Мобильные приложения
        ServiceFeature(service_id=services[6].id, text_ru="Работает на iOS и Android", text_en="Works on iOS and Android", sort_order=0),
        ServiceFeature(service_id=services[6].id, text_ru="Установка на домашний экран", text_en="Home screen installation", sort_order=1),
        ServiceFeature(service_id=services[6].id, text_ru="Офлайн-режим", text_en="Offline mode", sort_order=2),
        ServiceFeature(service_id=services[6].id, text_ru="Push уведомления", text_en="Push notifications", sort_order=3),
    ]
...
async def seed_advantages(session):
    advantages = [
        Advantage(
            icon="percent",
            title_ru="На 20–30% дешевле других агентств",
            title_en="20–30% cheaper than other agencies",
            description_ru="На 20–30% дешевле других агентств",
            description_en="20–30% cheaper than other agencies",
            sort_order=0,
        ),
        Advantage(
            icon="zap",
            title_ru="2 дня от обсуждения до старта работы",
            title_en="2 days from discussion to project kickoff",
            description_ru="2 дня от обсуждения до старта работы",
            description_en="2 days from discussion to project kickoff",
            sort_order=1,
        ),
        Advantage(
            icon="refresh-cw",
            title_ru="Полный цикл разработки от идеи до запуска",
            title_en="Full cycle development — from idea to launch",
            description_ru="Полный цикл разработки от идеи до запуска",
            description_en="Full cycle development — from idea to launch",
            sort_order=2,
        ),
        Advantage(
            icon="shield",
            title_ru="Соответствие 152-ФЗ о персональных данных",
            title_en="Compliance with 152-FZ personal data law",
            description_ru="Соответствие 152-ФЗ о персональных данных",
            description_en="Compliance with 152-FZ personal data law",
            sort_order=3,
        ),
        Advantage(
            icon="credit-card",
            title_ru="Поэтапная оплата, платите за результат",
            title_en="Pay as you go, pay for results",
            description_ru="Поэтапная оплата, платите за результат",
            description_en="Pay as you go, pay for results",
            sort_order=4,
        ),
        Advantage(
            icon="file-text",
            title_ru="Макет и ТЗ до подписания договора",
            title_en="Mockup and specs before signing the contract",
            description_ru="Макет и ТЗ до подписания договора",
            description_en="Mockup and specs before signing the contract",
            sort_order=5,
        ),
    ]
...
async def seed_client_types(session):
    ...
    client_types = [
        ClientType(
            ...
            subtitle_ru="Идеи, MVP, простые решения",
            subtitle_en="Ideas, MVP, simple solutions",
            description_ru="Есть идея, но не знаешь с чего начать, не проблема. Разберёмся вместе: выясним что реально нужно, предложим подходящее решение и реализуем. Без раздутых смет, без лишних сложностей.",
            description_en="Got an idea but don't know where to start, no problem. We'll figure it out together: understand what you actually need, suggest the right solution and build it. No bloated budgets, no unnecessary complexity.",
            sort_order=0,
        ),
        ClientType(
            title_ru="Малый/средний бизнес",
            title_en="Small/medium business",
            subtitle_ru="Автоматизация, онлайн-инструменты",
            subtitle_en="Automation, online tools",
            description_ru="Автоматизируем процессы, выводим в онлайн, делаем инструменты которые реально используются, а не пылятся после сдачи. Погружаемся в процессы, предлагаем что имеет смысл и остаёмся на связи после запуска.",
            description_en="We automate processes, take businesses online, and build tools that actually get used, not ones that collect dust after delivery. We go deep on your workflow, suggest what makes sense and stay in touch after launch.",
            sort_order=1,
        ),
        ClientType(
            title_ru="Корпорации",
            title_en="Corporations",
            subtitle_ru="Спецпроекты, Telegram Mini Apps",
            subtitle_en="Special projects, Telegram Mini Apps",
            description_ru="Разрабатываем спецпроекты и помогаем проверять гипотезы: интерактивные лендинги, мини-игры, квесты, Telegram Mini Apps. Нестандартный формат, быстрый запуск, высокое вовлечение аудитории.",
            description_en="We build special projects and help test hypotheses: interactive landing pages, mini-games, quests, Telegram Mini Apps. Non-standard format, fast launch, high user engagement.",
            sort_order=2,
        ),
    ]
...
async def seed_about_sections(session):
    ...
    sections = [
        AboutSection(
            title_ru="Кто мы",
            title_en="Who we are",
            description_ru="Мы небольшая команда — каждый проект ведёт конкретный человек, а не безликий менеджер. Специализируемся на Telegram Mini Apps, сайтах и корпоративных системах. Погружаемся в задачу, предлагаем решения, а не просто выполняем ТЗ.",
            description_en="We're a small team — every project has a real person behind it, not a faceless account manager. We specialize in Telegram Mini Apps, websites, and enterprise systems. We dig into the problem and bring ideas to the table, not just execute specs.",
            sort_order=0,
        ),
        AboutSection(
            title_ru="Наш принцип",
            title_en="Our Principle",
            description_ru="Нам важно одно — чтобы то, что мы делаем, реально работало и приносило результат. Не берём проекты ради галочки и не исчезаем после сдачи.",
            description_en="We care about one thing — that what we build actually works and delivers results. We don't take on projects just to check a box, and we don't disappear after launch.",
            sort_order=1,
        ),
    ]
...
async def seed_settings(session):
    settings_data = [
        SiteSetting(
            key="hero_title",
            value_ru="Разработка корпоративных систем и AI-решений",
            value_en="Development of enterprise systems and AI solutions",
        ),
        SiteSetting(
            key="hero_subtitle",
