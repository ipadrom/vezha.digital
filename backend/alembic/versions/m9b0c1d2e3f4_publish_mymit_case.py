"""Publish the reviewed MyMIT case without replacing an existing case.
...
def seed_case(connection, document):
    """Only insert MyMIT; an already-authored copy is never overwritten."""
    meta = deepcopy(document["meta"])
    if meta["slug"] != "mymit":
        raise ValueError("This migration only publishes the MyMIT case")
    schema = sa.MetaData()
    ...
    project_id = str(uuid5(NAMESPACE_URL, "https://vezha.digital/cases/mymit"))
    authored_blocks = []
    for index, source in enumerate(document["blocks"]):
        item = deepcopy(source)
        item["id"] = str(uuid5(NAMESPACE_URL, f"mymit-case-20260921-block-{index}"))
    ...
    connection.execute(
        revisions.insert().values(
            id=str(uuid5(NAMESPACE_URL, "mymit-case-20260921-initial-revision")),
