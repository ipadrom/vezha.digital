"""Publish the hidden MyMIT client introduction without touching other editorial content.
...
def load_block():
    path = Path(__file__).parents[1] / "data" / "mymit_client_block_20260922.json"
    return json.loads(path.read_text(encoding="utf-8"))


def patch_document(document):
    """Insert the client introduction after the cover and hide the duplicated hero name."""
    ...
    hero = next((block for block in blocks if block["type"] == "hero"), None)
    if hero is not None:
        hero["settings"] = {**hero["settings"], "show_project_name": False}
    return result
