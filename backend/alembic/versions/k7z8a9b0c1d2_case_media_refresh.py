"""Publish approved Zagorulko content and the SSAG hero palette.

Revision ID: k7z8a9b0c1d2
Revises: j6y7z8a9b0c1
"""
...
revision = "k7z8a9b0c1d2"
down_revision = "j6y7z8a9b0c1"
branch_labels = None
depends_on = None
...
def patch_document(document, slug):
    ...
    if slug == "gbu-process-automation":
        approved = load_document()
        result["meta"].update(approved["meta"])
        result["blocks"] = approved["blocks"]
    elif slug == "ssag":
        heroes = [b for b in result["blocks"] if b["type"] == "hero"]
        if len(heroes) != 1:
            raise ValueError("SSAG must have exactly one hero")
        heroes[0]["settings"].update(hero_background="#fffbf4", hero_text="#004e54")
    return result
...
def upgrade_cases(db):
    ...
    for project in db.execute(
        sa.select(projects).where(projects.c.slug.in_(["gbu-process-automation", "ssag"]))
    ).mappings():
        pid = project["id"]
        rows = list(
            db.execute(
        ...
        if public and updated_public != public:
            ...
            if project["slug"] == "gbu-process-automation":
